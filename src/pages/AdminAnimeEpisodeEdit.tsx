...
const AdminAnimeEpisodeEdit = () => {
  ...
  const { data: seasonData } = useQuery({
    queryKey: ["anime-season", id, seasonNumber],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("seasons")
        .select("*")
  ...
  const { data: episodeData, isLoading: episodeLoading } = useQuery({
    queryKey: ["anime-episode", seasonData?.id, episodeNumber],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("episodes")
        .select("*")
    ...
    enabled: !!seasonData?.id && !!episodeNumber,
  });
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Button
              variant="ghost"
              size="icon"
              onClick={() => navigate(`/admin/animes/${id}/edit`)}
            >
              <ArrowLeft className="h-4 w-4" />
            </Button>
            <div>
              <h1 className="text-2xl font-bold tracking-tight">
                Edit Episode {episodeNumber}: {episodeData.title}
              </h1>
              <p className="text-muted-foreground">
                {anime?.title} - Season {seasonNumber}
              </p>
            </div>
          </div>
...
export default AdminAnimeEpisodeEdit;
